...
class GeneralCustomExceptionHandlerTest {

	private GeneralCustomExceptionHandler handler = new GeneralCustomExceptionHandler();
	private I18nSupportTest i18nSupportTest = new I18nSupportTest();

	@Test
	void handleBadRequestTest() {
		CustomException exception = new CustomExceptionBuilder("Test01",
				CustomExceptionCategoryEnum.CLIENT_INVALID_PARAMETER_OR_DATA, null).build();
		HttpStatus respStatus = HttpStatus.BAD_REQUEST;
		ResponseError respError = prepareResponseError(respStatus, exception.getMessage());
		ResponseEntity<ResponseWrapper<ResponseError>> response = handler.handle(exception, i18nSupportTest.messageSource(),
		...
		Assertions.assertAll(() -> Assertions.assertNotNull(response, "Asserts that response is not null"),
				...
				() -> Assertions.assertNotNull(response.getStatusCode(), "Asserts that response status code is not null"),
				() -> Assertions.assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode(),
						"Asserts that response status code is 400"),
				() -> Assertions.assertNotNull(response.getBody().getData(), "Asserts that response body data is not null"),
				...
				() -> Assertions.assertEquals(respError.getCode(), ((ResponseError) response.getBody().getData()).getCode(),
	...
	void handleUnprocessableEntityTest() {
		CustomException exception = new CustomExceptionBuilder("Test02", CustomExceptionCategoryEnum.CLIENT_DATA_NOT_FOUND,
				null).build();
	...
	void handleInternalServerErrorTest() {
		CustomException exception = new CustomExceptionBuilder("Test03", CustomExceptionCategoryEnum.INTERNAL, null)
				.build();
	...
	private ResponseError prepareResponseError(HttpStatus status, String message) {
		return new ResponseError(status.series().value(), i18nSupportTest.messageSource()
				.getMessage(RestControllerAdvice.getDefaultMessageProperty(status), null, i18nSupportTest.locale()), message);
	}
}