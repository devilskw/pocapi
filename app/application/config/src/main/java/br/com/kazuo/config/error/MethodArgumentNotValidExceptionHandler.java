...
public class MethodArgumentNotValidExceptionHandler implements CustomExceptionHandler<MethodArgumentNotValidException> {
    ...
    @Override
    public ResponseEntity<ResponseWrapper<ResponseError>> handle(MethodArgumentNotValidException exception,
    ...
    public String getMessage(MethodArgumentNotValidException exception, MessageSource messageSource, Locale locale) {
        ...
        // "name"
        // exception.getBindingResult().getAllErrors().get(0).getArguments().get(0)
        // - args
        //
        // "baseProductDTO"
        // exception.getBindingResult().getAllErrors().get(0).getObjectName()
        //
        // exception.getBindingResult().getAllErrors().get(0).getArguments()[0].getCodes()
        // // 0 = baseProductDTO.name / 1 = name
        // exception.getBindingResult().getAllErrors().get(0).getArguments()[0].defaultMessage
        // // "name"
        //
        // exception.getBindingResult().getAllErrors().get(0).getCodes()[0] //
        // 0 = NotNull.baseProductDTO.name
        Object[] args = null;
        try {
            if (!exception.getBindingResult().getAllErrors().isEmpty()
                ...
                String msgProperty = new StringBuilder().append(RestControllerAdvice.MSGPREFIX)
                        .append(exception.getBindingResult().getAllErrors().get(0).getCodes()[0]).toString();
