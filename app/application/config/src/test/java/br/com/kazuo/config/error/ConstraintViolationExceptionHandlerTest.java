...
class ConstraintViolationExceptionHandlerTest {
    ...
    void handleConstraintViolationExceptionBadRequestWithoutMessagePropertyTest() {
        ...
        ResponseEntity<ResponseWrapper<ResponseError>> response = handler.handle(exception,
    ...
    void handleConstraintViolationExceptionBadRequestDefinedMessageTest() {
        ...
        args[0] = "1";
        ResponseError respError = this.prepareResponseError(respStatus, "Validation.Exception.Min.test", args,
    ...
    private ResponseError prepareResponseError(HttpStatus status, String message) {
        return new ResponseError(status.series().value(), i18nSupportTest.messageSource().getMessage(
                RestControllerAdvice.getDefaultMessageProperty(status), null, i18nSupportTest.locale()), message);
    }
    ...
    private ResponseError prepareResponseError(HttpStatus status, String msgProperty, Object[] args, String message) {
        return new ResponseError(status.series().value(),
                i18nSupportTest.messageSource().getMessage(msgProperty, args, i18nSupportTest.locale()), message);
    }
